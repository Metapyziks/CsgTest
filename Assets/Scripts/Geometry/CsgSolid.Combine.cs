...
namespace CsgTest.Geometry
{
    ...
    {
        [ThreadStatic]
        private static List<CsgConvexSolid> _sCombineList;

        public bool Combine( CsgConvexSolid solid, BrushOperator op )
        {
            _sCombineList ??= new List<CsgConvexSolid>();
            _sCombineList.Clear();
            _sCombineList.Add( solid );

            return Combine( _sCombineList, op );
        }

        public bool Combine( List<CsgConvexSolid> solids, BrushOperator op )
        {
            var changed = false;
            ...
            foreach ( var solid in solids )
            {
                ...
                for ( var polyIndex = _polyhedra.Count - 1; polyIndex >= 0; --polyIndex )
                {
                    ...
                    if ( solids.Contains( next ) ) continue;
